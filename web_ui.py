...
chiseled features""",
    ...
    with tab1:
        ...
        # Separate pipeline agents from editors
        pipeline_agents = {k: v for k, v in active_config.items() if not k.endswith('_editor')}
        editor_agents = {k: v for k, v in active_config.items() if k.endswith('_editor')}
        ...
        # Display editor agents
        if editor_agents:
            st.markdown("---")
            st.markdown("### ✏️ Editing Suite Agents")
            for agent_name, config in editor_agents.items():
                with st.expander(f"✏️ {agent_name.upper().replace('_', ' ')}", expanded=False):
